...
func requiredPoint(itm []item, byr *buyer) (canbuy bool) {	
	...
	if byr.point < bucketpoint {
		Println("마일리지가 %d점 부족합니다.", bucketpoint - byr.point)
		return false
	}
	return true
}
		...
		for i := 0; i < len(itm); i++ {
			if itm[i].name == index {
				if itm[i].amount < val { // 장바구니의 물품 총 개수가 판매하는 물품 개수보다 클 때
					Println("%s, %d개초과", itm[i].name, val-itm[i].amount)
					return false
				}
			}
		}
